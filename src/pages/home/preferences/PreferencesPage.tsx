...
const requestUpdatePreferences = async (
  id: string,
  preferences: string[],
  callback: () => void
) => {
...
const PreferencesPage: React.FC = () => {
  ...
  const { handleNavigation, acceptedTerm } = useHomeNavigation();
  ...
  return acceptedTerm ? (
    <Container maxWidth="xl">
      <Stack direction="row" alignItems="center" sx={{ marginTop: 1 }}>
        <Header variant="h5">
          Preferences
          <Typography variant="caption" sx={{ ml: 1, fontSize: "15px" }}>
            (Max 5 selections)
          </Typography>
        </Header>
      </Stack>
        ...
        {CATEGORIES.map((item, index) => {
          return (
            <Grid item xs={12} sm={6} md={4} key={item}>
              ...
              >
                <ImageBG
                  image={`/images/preferences/${item.replace(" ", "_")}.jpg`}
                >
                  <Typography
                    sx={{ background: "#57515133", p: 1, fontWeight: "bold" }}
                    variant="h5"
                    color="white"
                  >
                    {item}
